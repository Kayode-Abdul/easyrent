...
class PaymentWebhookController extends Controller
{
        ...
        try {
            ...
            if ($payment->status !== 'completed' && $payment->status !== 'success') {
                $payment->update([
                    ...
                    is_array($payment->payment_meta) ? $payment->payment_meta : json_decode($payment->payment_meta ?? '{}', true),
                    [
                        'webhook_data' => $data,
    ...
    private function updateBenefactorPaymentStatus($benefactorPayment, $status, $data)
    {
        $benefactorPayment->update([
            ...
            'payment_meta' => array_merge(
            is_array($benefactorPayment->payment_meta) ? $benefactorPayment->payment_meta : json_decode($benefactorPayment->payment_meta ?? '{}', true),
            [
                'webhook_data' => $data,
                'updated_via_webhook' => true,
                'webhook_timestamp' => now()->toISOString()
    ...
    private function triggerPostPaymentProcessing($payment)
    {
        try {
            ...
            // Update apartment status only if tenant is already assigned (not guest flow)
            if ($payment->apartment && $payment->tenant_id) {

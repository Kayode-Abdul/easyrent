        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                            ...
                            <tbody>
                                ...
                                <tr class="{{ !$rate->is_active ? 'table-secondary' : '' }}">
                                    ...
                                    <td>
                                        <span class="font-weight-bold text-success">{{
                                            number_format($rate->commission_percentage, 2) }}%</span>
                                    </td>
                                    <td>
                                        <small class="text-muted">{{ $rate->effective_from->format('M d, Y H:i')
                                            }}</small>
                                    </td>
                                    <td>
                                        @if($rate->effective_until)
                                        <small class="text-muted">{{ $rate->effective_until->format('M d, Y H:i')
                                            }}</small>
                                        @else
                                ...
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-4">
                                        <i class="fa fa-info-circle"></i> No commission rates found matching your
                                        criteria.
                                    </td>
                                </tr>
                                @endforelse
                    ...
                    <!-- Pagination -->
                    @if(isset($rates) && method_exists($rates, 'firstItem'))
                    <div class="text-muted">
                        Showing {{ $rates->firstItem() ?? 0 }} to {{ $rates->lastItem() ?? 0 }} of {{ $rates->total() }}
                        results
                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to deactivate this commission rate? This action will set the effective end date
                    to now and prevent it from being used for future calculations.</p>
            </div>

                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="align-self-center">
                                <i class="fa fa-file fa-2x opacity-75"></i>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <h3 class="mb-0">
                                    ...
                                    @else
                                        <i class="fa fa-exclamation-circle"></i> No Logs
                                    @endif
                                </h3>
                            </div>
                            <div class="align-self-center">
                                <i class="fa fa-heartbeat fa-2x opacity-75"></i>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        @if(count($logFiles) > 0)
                            <div class="list-group list-group-flush">
                                @foreach($logFiles as $index => $file)
                                    <a href="#"
                                        class="list-group-item list-group-item-action log-file-item {{ $index === 0 ? 'active' : '' }}"
                        ...
                        <div>
                            <div class="btn-group" role="group">
                                <button class="btn btn-sm btn-outline-secondary" onclick="toggleAutoRefresh()">
                                    <i class="fa fa-sync" id="auto-refresh-icon"></i>
                    ...
                    <div class="card-body p-0" style="height: 600px; overflow-y: auto;">
                        ...
                        <div id="log-content">
                            @if(count($recentLogs) > 0)
                                <pre class="p-3 mb-0"
                                    style="font-size: 12px; line-height: 1.4; color: #333; background: #f8f9fa;">@foreach($recentLogs as $log)<span class="log-line">{{ $log }}</span>
                                    @endforeach</pre>
                            @else
                                <div class="p-4 text-center text-muted">
                                    <i class="fa fa-file-alt fa-3x mb-3 d-block"></i>
        ...
        $.get(`{{ route('admin.logs.content') }}?file=${encodeURIComponent(filePath)}`)
            .done(function (data) {
                if (data.content) {
                    ...
                    $('#log-content').html('<div class="p-4 text-center text-muted"><i class="fa fa-file-alt fa-3x mb-3 d-block"></i><h6>Empty Log File</h6></div>');
                }
            })
            .fail(function () {
                $('#log-content').html('<div class="p-4 text-center text-danger"><i class="fa fa-exclamation-triangle fa-3x mb-3 d-block"></i><h6>Error Loading Log</h6><p class="small">Could not load log file content</p></div>');
            });
    }

        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('admin.roles.index') }}" class="btn btn-sm btn-secondary"><i
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <form method="POST" action="{{ route('admin.roles.assign.post') }}" id="modernRoleForm">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="modernUserInput" class="form-control"
                                        list="modernUserDatalist" placeholder="Start typing name or email"
                                        autocomplete="off" required>
                                    <datalist id="modernUserDatalist">
                                        @foreach($users as $u)
                                        <option value="{{ $u->first_name }} {{ $u->last_name }} ({{ $u->email }})">
                                        </option>
                                        @endforeach
                                    </datalist>
                                ...
                                <div class="form-group">
                                    <label>Role <span class="small text-muted"
                                            title="Some lifecycle roles are hidden and auto-managed">(managed
                                            list)</span></label>
                                    <select name="role_id" class="form-control" id="roleSelect" required>
                                        <option value="">-- Select Role --</option>
                                        @foreach(($assignableRoles ?? $allRoles) as $r)
                                        <option value="{{ $r->id }}" data-name="{{ strtolower($r->name) }}">{{
                                            $r->display_name ?? ucfirst(str_replace('_',' ', $r->name)) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="form-text text-muted">Hidden roles (tenant, landlord, marketer, super
                                        marketer, property manager, artisan) are granted by user actions.</small>
                                </div>

                                <div id="regionalScopeWrapper" class="border rounded p-3 mb-3" style="display:none;">
                                    <h6 class="mb-2">Assign Region(s)</h6>
                                    <p class="small text-muted mb-2">Select a state and optionally an LGA (city). Add
                                        multiple scopes if needed.</p>
                                    <div id="scopesContainer"></div>
                                    <button type="button" class="btn btn-sm btn-outline-primary" id="addScopeBtn"><i
                                            class="fa fa-plus"></i> Add Scope</button>
                                </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <form method="POST" action="{{ route('admin.roles.assign.post') }}">
                                ...
                                <div class="form-group">
                                    <label>User <span class="small text-muted">(type to search)</span></label>
                                    <input type="text" id="legacyUserInput" class="form-control"
                                        list="legacyUserDatalist" placeholder="Start typing name or email"
                                        autocomplete="off" required>
                                ...
                                <div id="legacyRegionalScopeWrapper" class="border rounded p-3 mb-3"
                                    style="display:none;">
                                    <h6 class="mb-2">Assign Region(s)</h6>
                                    <div id="legacyScopesContainer"></div>
                                    <button type="button" class="btn btn-sm btn-outline-primary"
                                        id="addLegacyScopeBtn"><i class="fa fa-plus"></i> Add Scope</button>
                                </div>
                    ...
                    <div class="row mt-3">
                        <div class="col-12">
                            ...
                            <div class="table-responsive" style="max-height:220px; overflow:auto;">
                                @if(session('success'))
                                <div style="padding:8px;background:#d1fae5;color:#065f46;margin-bottom:10px;">{{
                                    session('success') }}</div>
                                @endif
                                @if(session('error'))
                                <div style="padding:8px;background:#fee2e2;color:#991b1b;margin-bottom:10px;">{{
                                    session('error') }}</div>
                                @endif
                                <div
                                    style="display:flex;gap:12px;margin-bottom:12px;align-items:center;flex-wrap:wrap;">
                                    <form method="GET" action="{{ route('admin.roles.audits.export') }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.roles.audits.prune') }}"
                                        onsubmit="return confirm('Delete audit records older than the keep window?');"
                                        style="display:flex;gap:4px;align-items:center;">
                                        @csrf
                                        <label>Keep last
                                            <input type="number" name="keep_days" value="30" min="1" max="365"
                                                style="width:70px;"> days
                                        </label>
                                ...
                                <table class="table table-sm table-striped mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($a->role)
                                                {{ $a->role->display_name ?? ucfirst(str_replace('_',' ',
                                                $a->role->name)) }}
                                            ...
                                            <td>
                                                <span
                                                    class="badge badge-{{ $a->action === 'assigned' ? 'success' : ($a->action === 'blocked' ? 'danger' : 'secondary') }}">{{
                                                    $a->action }}</span>
                                            </td>
                                        ...
                                        @empty
                                        <tr>
                                            <td colspan="6" class="text-muted">No recent activity.</td>
                                        </tr>
                                        @endforelse
                                    </tbody>
                                </table>

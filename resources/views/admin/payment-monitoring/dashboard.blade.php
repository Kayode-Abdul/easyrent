...
@section('content')
    <div class="container-fluid">
        ...
        <div id="systemStatusAlert" class="row mb-4">
            <div class="col-12">
                <div class="alert alert-{{ $dashboardData['overview']['system_status'] === 'healthy' ? 'success' : ($dashboardData['overview']['system_status'] === 'critical' ? 'danger' : 'warning') }}"
                    role="alert">
                    <div class="d-flex align-items-center">
                        <i
                            class="fa fa-{{ $dashboardData['overview']['system_status'] === 'healthy' ? 'check-circle' : ($dashboardData['overview']['system_status'] === 'critical' ? 'exclamation-triangle' : 'exclamation-circle') }} me-2"></i>
                        <strong>System Status: {{ ucfirst($dashboardData['overview']['system_status']) }}</strong>
                        <span class="ms-auto">Last Updated: <span
                                id="lastUpdated">{{ $dashboardData['overview']['generated_at'] }}</span></span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="card border-warning">
                        <div class="card-header bg-warning text-dark">
                            <h5 class="card-title mb-0">
                                <i class="fa fa-exclamation-triangle"></i> Active Alerts ({{ count($dashboardData['alerts']) }})
                            </h5>
                        </div>
                        <div class="card-body">
                            @foreach($dashboardData['alerts'] as $alert)
                                <div
                                    class="alert alert-{{ $alert['severity'] === 'critical' ? 'danger' : ($alert['severity'] === 'warning' ? 'warning' : 'info') }} mb-2">
                                    <div class="d-flex justify-content-between align-items-start">
                                        ...
                                        <small
                                            class="text-muted">{{ \Carbon\Carbon::parse($alert['timestamp'])->diffForHumans() }}</small>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="card text-center">
                    <div class="card-body">
                        ...
                        <small class="text-success">
                            <i class="fa fa-arrow-up"></i> {{ $dashboardData['performance']['success_rate'] }}% Success Rate
                        </small>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="card text-center">
                    <div class="card-body">
                        ...
                        <small
                            class="{{ $dashboardData['performance']['avg_execution_time_ms'] <= 100 ? 'text-success' : 'text-warning' }}">
                            <i
                                class="fa fa-{{ $dashboardData['performance']['avg_execution_time_ms'] <= 100 ? 'check' : 'exclamation-triangle' }}"></i>
            ...
            <div class="col-md-3">
                <div class="card text-center">
                    <div class="card-body">
                        ...
                        <small
                            class="{{ $dashboardData['accuracy']['accuracy_rate'] >= 98 ? 'text-success' : 'text-warning' }}">
                            <i
                                class="fa fa-{{ $dashboardData['accuracy']['accuracy_rate'] >= 98 ? 'check' : 'exclamation-triangle' }}"></i>
                ...
                <div class="card text-center">
                    <div class="card-body">
                        ...
                        <small class="{{ $dashboardData['errors']['error_rate'] <= 1 ? 'text-success' : 'text-danger' }}">
                            <i class="fa fa-{{ $dashboardData['errors']['error_rate'] <= 1 ? 'check' : 'times' }}"></i>
                        ...
                        <table class="table table-sm">
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-end">
                                        {{ number_format($dashboardData['performance']['total_calculations']) }}</td>
                                </tr>
                                <tr>
                                    <td>Successful Calculations</td>
                                    <td class="text-end">
                                        {{ number_format($dashboardData['performance']['successful_calculations']) }}</td>
                                </tr>
                                <tr>
                                    <td>Failed Calculations</td>
                                    <td class="text-end">
                                        {{ number_format($dashboardData['performance']['failed_calculations']) }}</td>
                                </tr>
                                <tr>
                                    <td>Slow Calculations</td>
                                    <td class="text-end">
                                        {{ number_format($dashboardData['performance']['slow_calculations']) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="table table-sm">
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-end">
                                        {{ number_format($dashboardData['accuracy']['total_verified_calculations']) }}</td>
                                </tr>
                                <tr>
                                    <td>Accurate Calculations</td>
                                    <td class="text-end">
                                        {{ number_format($dashboardData['accuracy']['accurate_calculations']) }}</td>
                                </tr>
                                <tr>
                                    <td>Average Deviation</td>
                                    <td class="text-end">
                                        ${{ number_format($dashboardData['accuracy']['avg_deviation'], 2) }}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        {{ number_format($dashboardData['accuracy']['high_value_calculations']) }}</td>
                                </tr>
                                <tr>
                                    <td>Suspicious Calculations</td>
                                    <td class="text-end">
                                        {{ number_format($dashboardData['accuracy']['suspicious_calculations']) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                            ...
                            <table class="table table-sm">
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            {{ number_format($dashboardData['pricing_configuration_usage']['total_pricing_type']) }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Monthly Pricing Type</td>
                                        <td class="text-end">
                                            {{ number_format($dashboardData['pricing_configuration_usage']['monthly_pricing_type']) }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Fallback Usage</td>
                                        <td class="text-end">
                                            {{ number_format($dashboardData['pricing_configuration_usage']['fallback_usage']) }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Configuration Changes</td>
                                        <td class="text-end">
                                            {{ number_format($dashboardData['pricing_configuration_usage']['configuration_changes']) }}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
    ...
    <div id="autoRefreshIndicator" class="position-fixed bottom-0 end-0 m-3" style="display: none;">
        <div class="alert alert-info alert-dismissible">
            <i class="fa fa-sync-alt fa-spin"></i> Auto-refreshing...
        </div>
    </div>
@endsection

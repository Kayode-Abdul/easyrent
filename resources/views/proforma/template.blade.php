...
@section('content')
    ...
    <div class="content" style="max-width:700px;margin:90px auto;font-family:sans-serif;">
        @if($isPaid)
            <div style="background:#d1ecf1;color:#0c5460;padding:15px;border-radius:8px;margin-bottom:20px;border-left:5px solid #0c5460;display:flex;align-items:center;">
                <i class="fa fa-check-circle" style="font-size:24px;margin-right:15px;"></i>
                <div>
                    <strong>Payment Already Settled!</strong><br>
                    This invoice has been successfully paid on {{ $payment->paid_at->format('M d, Y') }}.
                    <a href="{{ route('payment.receipt', $payment->id) }}" style="color:#0c5460;font-weight:bold;text-decoration:underline;">View Receipt</a>
                </div>
            </div>
        @elseif($isOccupied)
             <div style="background:#fff3cd;color:#856404;padding:15px;border-radius:8px;margin-bottom:20px;border-left:5px solid #856404;display:flex;align-items:center;">
                <i class="fa fa-exclamation-triangle" style="font-size:24px;margin-right:15px;"></i>
                <div>
                    <strong>Apartment No Longer Available</strong><br>
                    This apartment has been taken by another tenant. This invoice is no longer valid for payment.
                </div>
            </div>
        @endif
        ...
        <table style="width:100%;margin-bottom:20px;">
            <tr>
                ...
                <td style="text-align:right;">
                    ...
                    <strong>Date:</strong>
                    {{ $proforma->created_at ? $proforma->created_at->format('d/m/Y') : now()->format('d/m/Y') }}<br>
        ...
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="background:#f5f5f5;">
                    ...
                    <th style="border:1px solid #ccc;padding:8px;text-align:right;">Amount
                        ({{ $proforma->currency->symbol ?? format_money(0)->getSymbol() }})</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    ...
                    <td style="border:1px solid #ccc;padding:8px;text-align:right;">
                        {{ format_money($proforma->amount ?? optional($proforma->apartment)->amount ?? 0, $proforma->currency) }}
                    </td>
                </tr>
                @if($proforma->security_deposit)
                    <tr>
                        ...
                        <td style="border:1px solid #ccc;padding:8px;text-align:right;">
                            {{ format_money($proforma->security_deposit, $proforma->currency) }}</td>
                    </tr>
                @endif
                @if($proforma->water)
                    <tr>
                        ...
                        <td style="border:1px solid #ccc;padding:8px;text-align:right;">
                            {{ format_money($proforma->water, $proforma->currency) }}</td>
                    </tr>
                @endif
                @if($proforma->internet)
                    <tr>
                        ...
                        <td style="border:1px solid #ccc;padding:8px;text-align:right;">
                            {{ format_money($proforma->internet, $proforma->currency) }}</td>
                    </tr>
                @endif
                @if($proforma->generator)
                    <tr>
                        ...
                        <td style="border:1px solid #ccc;padding:8px;text-align:right;">
                            {{ format_money($proforma->generator, $proforma->currency) }}</td>
                    </tr>
                @endif
                @if($proforma->other_charges_desc || $proforma->other_charges_amount)
                    <tr>
                        ...
                        <td style="border:1px solid #ccc;padding:8px;text-align:right;">
                            {{ format_money($proforma->other_charges_amount, $proforma->currency) }}</td>
                    </tr>
                @endif
                <tr style="font-weight:bold;background:#f5f5f5;">
                    <td style="border:1px solid #ccc;padding:8px;">Total</td>
                    <td style="border:1px solid #ccc;padding:8px;text-align:right;">
                        {{ format_money($proforma->total, $proforma->currency) }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <p style="margin-top:20px;font-size:14px;color:#555;">
            <strong>Note:</strong> This is an estimate/statement before payment. Upon receipt of the payment, a formal
            receipt w

            @if(auth()->user()->user_id === $proforma->tenant_id && $proforma->status !== \App\Models\ProfomaReceipt::STATUS_REJECTED && !$isPaid)
                    ...
                    @if($proforma->status === \App\Models\ProfomaReceipt::STATUS_NEW)
                        @if($isOccupied)
                            <div style="background:#fff3cd;padding:15px;border-radius:5px;margin-bottom:10px;">
                                <p style="margin:0;color:#856404;font-weight:bold;">⚠️ Apartment No Longer Available</p>
                                <p style="margin:5px 0 0 0;color:#856404;font-size:14px;">This unit has been rented out to another tenant.</p>
                            </div>
                        @else
                            <button id="accept-proforma" class="btn-accept"
                                style="background:#28a745;color:white;border:none;padding:10px 20px;margin-right:10px;border-radius:5px;cursor:pointer;">
                                Accept Proforma
                            </button>
                            <button id="reject-proforma" class="btn-reject"
                                style="background:#dc3545;color:white;border:none;padding:10px 20px;margin-right:10px;border-radius:5px;cursor:pointer;">
                                Reject Proforma
                            </button>
                            <button id="invite-benefactor" class="btn-benefactor"
                                style="background:#6f42c1;color:white;border:none;padding:10px 20px;border-radius:5px;cursor:pointer;">
                                <i class="fa fa-user-plus"></i> Invite Someone to Pay
                            </button>
                        @endif
                    @elseif($proforma->status === \App\Models\ProfomaReceipt::STATUS_CONFIRMED)
                        ...
                        @else
                            ...
                            <div style="text-align:center;margin-top:20px;">
                                @if($isOccupied)
                                    <button class="btn-payment" disabled
                                        style="background:#ccc;color:white;border:none;padding:12px 30px;border-radius:5px;display:inline-block;font-weight:bold;cursor:not-allowed;">
                                        🚫 Apartment Unavailable
                                    </button>
                                @else
                                    <a href="{{ route('proforma.payment.form', ['id' => $proforma->id]) }}" class="btn-payment"
                    ...
                    <div style="text-align:center;">
                        ...
                        <div style="background:#f8f9fa;padding:20px;border-radius:10px;margin-bottom:20px;text-align:left;">
                            <h6 style="margin:0 0 15px 0;color:#333;font-size:16px;text-align:center;">
                                <i class="fa fa-info-circle" style="margin-right:8px;color:#007bff;"></i>Payment Details
                            </h6>
                ...
                Swal.fire({
                    ...
                    <div style="text-align:left;">
                        ...
                        <p style="margin-top:15px;color:#6c757d;font-size:14px;">
                            <i class="fa fa-info-circle"></i> You can now paste this link in any messaging app, social media, or email to share with your benefactor.
                        </p>
                    </div>
                `,
                    icon: 'success',

...
@section('content')
    ...
    <div class="content">
        ...
        <div class="row">
            <div class="col-md-12">
                <h4 class="card-title mb-0" style="margin-top: 0;">
        ...
        <div class="row">
                        ...
                        <form action="{{ route('complaints.store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="form-group">
                                ...
                                <select name="apartment_id" id="apartment_id"
                                    class="form-control @error('apartment_id') is-invalid @enderror" required>
                                    <option value="">Choose your apartment...</option>
                                    @foreach($apartments as $apartment)
                                                                    <option value="{{ $apartment->apartment_id }}" {{ old(
                                            'apartment_id',
                                            $selectedApartment
                                        ) == $apartment->apartment_id ? 'selected' : '' }}
                                                                        data-property="{{ $apartment->property->address }}" data-landlord="{{ $apartment->owner->first_name }} {{ $apartment->owner->last_name
                                                                                                                                                }}">
                                ...
                                @enderror
                                <small class="form-text text-muted">Select the apartment where the issue is
                                    occurring.</small>
                            </div>
                                                                ...
                                                                <div class="col-md-6 col-lg-4 mb-3">
                                                                    <div class="category-option" data-category="{{ $category->id }}">
                                                                        <input type="radio" name="category_id" value="{{ $category->id }}"
                                                                            id="category_{{ $category->id }}" {{ old('category_id') == $category->id ?
                                        'checked' : '' }} class="d-none category-radio">
                                                                        <label for="category_{{ $category->id }}" class="category-card">
                                                                            <div class="text-center p-3">
                                                                                <i class="{{ $category->icon }} category-icon text-{{ $category->priority_color }}"
                            ...
                            <div class="form-group">
                                ...
                                <select name="priority" id="priority"
                                    ...
                                    <option value="low" {{ old('priority') === 'low' ? 'selected' : '' }}>Low - Can wait a few
                                        days</option>
                                    <option value="medium" {{ old('priority') === 'medium' ? 'selected' : '' }}>Medium -
                                        Should be addressed soon</option>
                                    <option value="high" {{ old('priority') === 'high' ? 'selected' : '' }}>High - Needs
                                        attention within 24 hours</option>
                                    <option value="urgent" {{ old('priority') === 'urgent' ? 'selected' : '' }}>Urgent -
                                        Immediate attention required</option>
                                </select>
                                @error('priority')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="title" id="title"
                                    ...
                                    placeholder="Brief description of the issue" maxlength="255" required>
                                @error('title')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="form-text text-muted">Minimum 10 characters. The more details you provide, the
                                    faster we can resolve your issue.</small>
                            </div>
            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <p class="small text-muted">Photos help us understand the issue better and resolve it faster.
                            </p>
                        </div>

                        <div class="mb-3">
                            <h6><i class="nc-icon nc-paper text-success"></i> Be Specific</h6>
                            <p class="small text-muted">Include details like when the issue started, how often it occurs,
                                and what you've tried.</p>
                        </div>

                        <div class="mb-3">
                            <h6><i class="nc-icon nc-time-alarm text-warning"></i> Set Correct Priority</h6>
                            <p class="small text-muted">Choose the right priority level to ensure appropriate response time.
                            </p>
                        </div>

                        <div class="mb-3">
                            <h6><i class="nc-icon nc-chat-33 text-primary"></i> Stay Updated</h6>
                            <p class="small text-muted">You'll receive email notifications about updates to your complaint.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <p class="small">For urgent issues that pose immediate safety risks (gas leaks, electrical hazards,
                            security breaches), contact emergency services or your landlord directly:</p>

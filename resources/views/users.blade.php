                ...
                <div class="row">
                    <div class="col-12"> <!-- Admin information for role management -->
                        ...
                        <div class="alert alert-info mb-4">
                            ...
                            <ul>
                                <li>Click the <i class="fa fa-user-plus"></i> icon next to a user to directly access
                                    their role management</li>
                                <li>Go to <a href="{{ route('admin.roles.index') }}" class="alert-link">Role
                                        Management</a> to see all roles and manage users by role</li>
                            </ul>
                        </div>
                        @endif
                        ...
                        <table class="table" id="usersTable">
                            ...
                            <tbody> @php
                                $roles = ['Landlord','Tenant','Artisan','Property Manager', 'Marketer', 'Regional
                                Manager'];
                                @endphp
                                @if (auth()->check())
                                ...
                                <tr data-user-id="{{ $user->user_id }}">
                                    <td class="images">
                                        <div class="avatar">
                                            <img class="img-circle img-no-padding border-gray"
                                                src="{{ $user->photo ? asset($user->photo) : asset('assets/images/default-avatar.png') }}"
                                                alt="...">
                                    ...
                                    <td class="action">
                                        <a href="{{ route('users.profile', ['id' => $user->user_id]) }}"
                                            class="view-user"><i class="fa fa-eye" aria-hidden="true"
                                                title="View details"></i></a>

                                        @if(auth()->check() && auth()->user()->admin == 1)
                                        <!-- Admin specific actions for role management -->
                                        <a href="{{ route('users.profile', ['id' => $user->user_id]) }}#role-management"
                                            class="btn btn-primary btn-sm ml-2" title="Manage Role">
                                        ...
                                        ($authUser->isLandlord() && $authUser->isLandlordOf($user)) ||
                                        ($authUser->isAgent() && $authUser->isLandlordOf($user)) ||
                                        // Tenant viewing their landlord/agent
                                        ($authUser->isTenant() && (
                                        ($user->isLandlord() && $authUser->isTenantOf($user))
                                        || ($user->isAgent() && $authUser->isTenantOf($user))
                                        ))
                                        )
                                        <a href="{{ url('/messages/compose?to=' . $user->user_id) }}"
                                            class="btn btn-success btn-sm ml-2" title="Message">

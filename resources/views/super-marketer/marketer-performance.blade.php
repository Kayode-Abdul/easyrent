                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center">
                                <div class="me-3">
                                    @if($marketer->photo)
                                        <img src="{{ asset('storage/' . $marketer->photo) }}" alt="{{ $marketer->first_name }}"
                                ...
                                <div>
                                    <h4 class="mb-1">
                                        ...
                                        <span
                                            class="badge bg-{{ $marketer->marketer_status === 'active' ? 'success' : 'warning' }} ms-2">
                                            {{ ucfirst($marketer->marketer_status) }}
                                        </span>
                                    </h4>
                                    <p class="text-muted mb-0">
                                        <i class="fa fa-envelope me-1"></i>{{ $marketer->email }}
                                        @if($marketer->phone)
                                            <span class="ms-3"><i class="fa fa-phone me-1"></i>{{ $marketer->phone }}</span>
                                        @endif
                                    </p>
                                    @if($marketer->marketerProfile && $marketer->marketerProfile->business_name)
                                        <p class="text-muted mb-0">
                                            <i class="fa fa-building me-1"></i>{{ $marketer->marketerProfile->business_name }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                            <div>
                                <a href="{{ route('super-marketer.referred-marketers') }}"
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="text-end">
                                <small class="text-success">{{ $performanceMetrics['best_month']['count'] }}
                                    referrals</small>
                            </div>
                        </div>
                        <div class="mb-3">
                                        ...
                                        @foreach($commissionHistory->take(10) as $payment)
                                                                <tr>
                                                                    ...
                                                                    <td class="font-weight-bold">
                                                                        {{ format_money($payment->total_amount, ($payment->currency->code ?? null)) }}
                                                                    </td>
                                                                    <td>
                                                                        <span class="badge bg-{{ 
                                    ...
                                    <tbody>
                                        @foreach($referralChains as $chain)
                                            <tr>
                                                ...
                                                <td>
                                                    <span
                                                        class="badge bg-{{ $chain->status === 'active' ? 'success' : 'warning' }}">
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <div class="me-2">
                                                            <i class="fa fa-crown text-warning" title="Super Marketer"></i>
                                                        </div>
                                                        <div class="me-2">
                                                            <i class="fa fa-arrow-right text-muted"></i>
                                                        </div>
                                                        <div class="me-2">
                                                            <i class="fa fa-user text-primary" title="Marketer"></i>
                                                        </div>
                                                        <div class="me-2">
                                                            <i class="fa fa-arrow-right text-muted"></i>
                                                        </div>
                                                        <div>
                                                            <i class="fa fa-home text-success" title="Landlord"></i>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <p><strong>Business Name:</strong> {{ $marketer->marketerProfile->business_name ?? 'N/A' }}
                                    </p>
                                    <p><strong>Business Type:</strong> {{ $marketer->marketerProfile->business_type ?? 'N/A' }}
                                    </p>
                                    <p><strong>Years of Experience:</strong>
                                        {{ $marketer->marketerProfile->years_of_experience ?? 'N/A' }} years</p>
                                    <p><strong>Marketing Channels:</strong>
                                        {{ $marketer->marketerProfile->marketing_channels ?? 'N/A' }}</p>
                                </div>
                                <div class="col-md-6">
                                    <h6>Contact & Preferences</h6>
                                    <p><strong>Website:</strong>
                                        @if($marketer->marketerProfile->website)
                                            <a href="{{ $marketer->marketerProfile->website }}"
                                                target="_blank">{{ $marketer->marketerProfile->website }}</a>
                                        @else
                                            N/A
                                        @endif
                                    </p>
                                    <p><strong>Social Media:</strong>
                                        {{ $marketer->marketerProfile->social_media_handles ?? 'N/A' }}</p>
                                    ...
                                    <p><strong>KYC Status:</strong>
                                        <span
                                            class="badge bg-{{ $marketer->marketerProfile->kyc_status === 'approved' ? 'success' : 'warning' }}">

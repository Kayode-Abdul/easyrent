...
@section('content')
                    ...
                    <div class="col-lg-3 col-md-6 col-sm-6 col-6">
                        <div class="card card-stats">
                            ...
                            <div class="card-footer">
                                <hr>
                                <div class="stats">
                                    <i class="fa fa-shopping-cart"></i> All time revenue
                                </div>
                            </div>
                        </div>
                    </div>
                                                    ...
                                                    @foreach($commissionData['recent_payments'] as $payment)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                @if($commission > 0)
                                                                                        <span
                                                                                            class="text-warning">{{ format_money($commission, $payment->currency) }}</span>
                                                                                        <small class="text-muted">({{ number_format(($commission / $payment->amount)
                                                                    * 100, 2) }}%)</small>
                                                                @else
                                                                    <span class="text-muted">{{ format_money(0, $payment->currency) }}</span>
                                                                @endif
                                                            </td>
                                                            <td class="text-success">{{ format_money($netAmount, $payment->currency) }}</td>
                                                            <td>
            ...
            @elseif($mode === 'tenant')
                    <div class="row">
                        ...
                        @php
                            ...
                            // Get payment statistics for tenant (Simplified, counts only)
                            $tenantPaymentsCount = \App\Models\Payment::where('tenant_id', auth()->user()->user_id)->count();
                            $pendingPayments = \App\Models\Payment::where('tenant_id', auth()->user()->user_id)->where('status', 'pending')->count();
                            $overduePayments = \App\Models\Payment::where('tenant_id', auth()->user()->user_id)->where('status', 'pending')->where('due_date', '<', now())->count();
                            $upcomingPayments = \App\Models\Payment::where('tenant_id', auth()->user()->user_id)->where('status', 'pending')->where('due_date', '>=', now())->where('due_date', '<=', now()->addDays(7))->count();
                        @endphp
                                            ...
                                            <tbody>
                                                @foreach($myApartment as $apartment)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <strong>{{ format_money($apartment->amount ?? 0, $apartment->currency ?? ($apartment->property->currency ?? null)) }}</strong><br>
                ...
                <div class="modal-body">
                    ...
                    <form method="post" id="propertyForm" class="p-3">
                        ...
                        <div class="form-group">
                            ...
                            <select name="state_id" id="states" class="form-control" onchange="getCities()">
                                <option value="" disabled="disabled" selected>Select State</option>
                            </select>
                            <input type="hidden" name="state" id="state_name">
                        </div>
                        <div class="form-group">
                            <label for="cities" id="cityLabel">City/L.G.A</label>
                            <select name="lga_id" id="cities" class="form-control" onchange="updateCityName()">
                                <option value="" disabled="disabled" selected>Select City/L.G.A</option>
                            </select>
    ...
    <script>
        ...
        function getCities() {
            ...
            if (!selectedStateId || !cachedModalLocationData) return;

            const found = cachedModalLocationData.find(s => s.id == selectedStateId);
            if (found && found.lgas) {
